...
import 'package:bgisp/api/ExecutiveCommittee/executive_committee_api.dart';
...
class _ExecutiveCommitteePageState extends State<ExecutiveCommitteePage> {
  ...
  List<ExecutiveCommittee> _committeeMembers = [];
  List<ExecutiveCommittee> _filteredMembers = [];
  List<ExecutiveCommittee> _paginatedMembers = [];
  ...
  Future<void> _fetchCommitteeMembers() async {
    ...
    try {
      final members = await ExecutiveCommitteeApi.getExecutiveCommittees(
  ...
  String _getImageUrl(ExecutiveCommittee member) {
    if (member.imagePath != null && member.imagePath!.isNotEmpty) {
      return '${ExecutiveCommitteeApi.baseUrl}${member.imagePath}';
    }
    return '';
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _languageHelper.translate('executive_committee_page'),
                    ...
                    Row(
                      ...
                      children: [
                        Column(
                          ...
                          children: [
                            Text(
                              _languageHelper.translate('executive_committee_members'),
                              style: const TextStyle(
                                color: Colors.white,
                                fontSize: 14,
                              ),
  ...
  Widget _buildCommitteeMemberCard(ExecutiveCommittee member) {
  ...
  void _showMemberDetails(ExecutiveCommittee member) {
